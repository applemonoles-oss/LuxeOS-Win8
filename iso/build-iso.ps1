...
$toolsSrc = Join-Path $projRoot "tools"
...
if (Test-Path $esd -and -not (Test-Path $wim)) {
    ...
    dism.exe /Export-Image /SourceImageFile:$esd /SourceIndex:1 /DestinationImageFile:$wim /DestinationName:"Windows 8.1 Pro (LuxeOS)" /Compress:max | Out-Null
}
...
Copy-Item (Join-Path $projRoot "iso\SetupComplete.cmd") $scripts -Force
Copy-Item (Join-Path $projRoot "iso\postinstall.ps1")  $scripts -Force

Msg "Committing image..."
dism.exe /Unmount-Image /MountDir:$mount /Commit

# 4) drop autounattend.xml at ISO root
Copy-Item (Join-Path $projRoot "iso\autounattend.xml") $iso -Force
